...
```{r}
happy |> 
  ______(country_name=="United States") |> 
  ggplot(___(year,happiness)) +
  geom_line()+
  geom______()
```

2.  Create a scatterplot showing the relationship between life expectancy and happiness for all countries in all years.

```{r}
happy |> 
  ggplot(aes(_______,_______))+
  geom_point()
```

3.   Recreate the figure above but use `col=continent=="Europe"` to assign the point color based on whether the data point is for a country in Europe.
...
happy |> 
  ...
  geom_col(aes(fill=country_name=="United States"))+
  coord_flip()
